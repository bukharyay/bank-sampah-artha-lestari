...
<?php foreach ($data_harga_sampah as $dhs) : ?>
	<div class="modal fade" id="hargaUpdate<?= $dhs->id_harga ?>" tabindex="-1" aria-labelledby="hargaUpdate<?= $dhs->id_harga ?>Label" style="display: none;" aria-hidden="true">
		<div class="modal-dialog modal-md" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title text-info" id="hargaUpdate<?= $dhs->id_harga ?>Label">Harga Sampah <i class="mdi mdi-pencil text-info align-middle"></i></h5>
				...
				<div class="modal-body">
					...
					<form class="forms-sample" id="hargaUpdate<?= $dhs->id_harga ?>" action="<?= base_url('Edit-Harga-Sampah/') . $dhs->id_sampah ?>" method="POST">
						...
						<div class="form-group <?= form_error('harga_per_kg') ? 'has-danger' : '' ?>">
							...
							<input type="number" class="form-control" id="harga_per_kg" name="harga_per_kg" value="<?= $dhs->harga_per_kg ?>" placeholder="Harga Sampah" required>
							<small class="text-small">*Harga per Kg</small>
							<?= form_error('harga_per_kg', '<label class="error mt-2 text-danger">', '</label>'); ?>
						</div>


						<div class="form-group <?= form_error('periode') ? 'has-danger' : '', form_error('id_sampah') ? 'has-danger' : '' ?>">
							<label for="periode">Periode terakhir update</label>
							<input type="date" class="form-control" id="periode" name="periode" value="<?= $dhs->periode ?>" placeholder="Periode" required min="<?= date('Y-m-d', strtotime('-1 week')) ?>" max="<?= date('Y-m-d') ?>">
