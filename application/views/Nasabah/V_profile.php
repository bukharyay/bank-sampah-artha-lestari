<div class="main-panel">
      ...
      <div class="col-12 col-md-6 col-lg-6 grid-margin grid-margin-md-0 stretch-card">
        <div class="card">
          <div class="card-body text-center">
            ...
            <?php $CI = &get_instance (); ?>
            <div>
              <img src="<?= base_url ( 'assets/profile/avatars/' ) . $this->user_session->avatar ?>"
  ...
  <div class="modal fade" id="nasabahEditModal" tabindex="-1" aria-labelledby="nasabahEditModalLabel"
       style="display: none;" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title text-warning" id="nasabahEditModalLabel">Edit Pengguna <i
        ...
        <div class="modal-body">
          <form class="forms-sample" id="formEditNasabah" action="
                <?= base_url ( 'Update-Profile/' ) . $this->user_session->id_user ?>" method="POST"
            ...
            <div class="row justify-content-center align-items-center w-auto mx-auto ">
              <div class="col-12 col-sm-12 col-md-10 col-lg-6">
                ...
                <div class="text-center mb-3">
                  <div class="d-flex justify-content-center ">
                    <img id="selectedAvatar<?= $this->user_session->id_user ?>"
                         src="<?= base_url ( 'assets/profile/avatars/' . ( $this->user_session->avatar ?? 'default.png' ) ) ?>"
                    ...
                    <div class="d-flex justify-content-center mt-4">
                      <div class="btn btn-sm btn-primary btn-rounded">
                        <label class="form-label text-white m-1" for="avatar<?= $this->user_session->id_user ?>">Ganti
                          Foto</label>
                        <input type="file" class="form-control d-none" id="avatar<?= $this->user_session->id_user ?>"
                               name="avatar"
                               onchange="displaySelectedImage(event, 'selectedAvatar<?= $this->user_session->id_user ?>')" />
                      </div>
                    </div>
              ...
              <div class="col-12 col-sm-12 col-md-10 col-lg-6">
                <div class="form-group">
                  <label
                         for="dibuat"><?= ! empty ( $this->user_session->role ) ? ucfirst ( $this->user_session->role ) : '' ?>
                    terdaftar sejak
                  </label>
                  <input type="text" class="form-control" id="dibuat<?= $this->user_session->id_user ?>"
                         value="<?= $this->user_session->role == 'nasabah' ? $CI->format_waktu ( $this->user_session->nasabah_created_at ) : $CI->format_waktu ( $this->user_session->user_created_at ) ?>"
                         placeholder="" readonly>
                </div>
                <div class="form-group">
                  <label
                         for="diupdate"><?= ! empty ( $this->user_session->role ) ? ucfirst ( $this->user_session->role ) : '' ?>
                    update terakhir
                  </label>
                  <input type="text" class="form-control" id="diupdate<?= $this->user_session->id_user ?>"
                         value="<?= $this->user_session->role == 'nasabah' ? $CI->format_waktu ( $this->user_session->nasabah_updated_at ) : $CI->format_waktu ( $this->user_session->user_updated_at ) ?>"
                         placeholder="" readonly>
                </div>
                <div class="form-group <?= form_error ( 'role' ) ? 'has-danger' : '' ?>">
                  <label for="role">Role</label>
                  <select class="form-control form-select" id="role<?= $this->user_session->id_user ?>" name="role"
                          readonly>
            ...
            <div class="row justify-content-center w-auto mx-auto">
              ...
              <div class="col-12 col-sm-12 col-md-10 col-lg-6">
                <div class="form-group <?= form_error ( 'username' ) ? 'has-danger' : '' ?>  ">
                  ...
                  <input type="text" class="form-control" id="username<?= $this->user_session->id_user ?>"
                         name="username" value="<?= $this->user_session->username ?>" placeholder="Username" required>
                  <?= form_error ( 'username', '<label class="error mt-2 text-danger">', '</label>' ); ?>
                </div>

                <div class="form-group <?= form_error ( 'email' ) ? 'has-danger' : '' ?>">
                  <label for="email">Email address</label>
                  <input type="email" class="form-control" id="email<?= $this->user_session->id_user ?>" name="email"
                         value="<?= $this->user_session->email ?>" placeholder="Email" required>
                ...
                <?php if ( $this->user_session->role == 'nasabah' ) : ?>
                  <div class="form-group <?= form_error ( 'password' ) ? 'has-danger' : '' ?>">
                    ...
                    <input type="password" class="form-control" id="password<?= $this->user_session->id_user ?>"
                           name="password" value="" placeholder="Password" autocomplete="on">
                  ...
                  <div class="form-group <?= form_error ( 'password_confirmation' ) ? 'has-danger' : '' ?>">
                    ...
                    <input type="password" class="form-control "
                           id="password_confirmation<?= $this->user_session->id_user ?>" name="password_confirmation"
              ...
              <div class="col-12 col-sm-12 col-md-10 col-lg-6">
                <?php if ( $this->user_session->role == 'nasabah' ) : ?>
                  ...
                  <div class="form-group <?= form_error ( 'id_rt' ) ? 'has-danger' : '' ?>">
                    ...
                    <select class="form-control form-select" id="id_rt<?= $this->user_session->id_user ?>" name="id_rt"
                            readonly>
                  ...
                  <div class="form-group <?= form_error ( 'nama' ) ? 'has-danger' : '' ?>">
                    ...
                    <input type="text" class="form-control" id="nama<?= $this->user_session->id_user ?>" name="nama"
                           value="<?= $this->user_session->nama ?? $this->user_session->nama ?>"
                  ...
                  <div class="form-group <?= form_error ( 'no_telfon' ) ? 'has-danger' : '' ?>">
                    ...
                    <input type="number" class="form-control" id="no_telfon<?= $this->user_session->id_user ?>"
                           name="no_telfon"
                           value="<?= $this->user_session->no_telfon ?? $this->user_session->no_telfon ?>"
                  ...
                  <div class="form-group">
                    ...
                    <textarea class="form-control h-50" rows="5" name="alamat"
                              id="alamat<?= $this->user_session->id_user ?>"><?= $this->user_session->alamat ?? $this->user_session->alamat ?></textarea>
                  </div>
                <?php else : ?>
                  <div class="form-group <?= form_error ( 'password' ) ? 'has-danger' : '' ?>">
                    <label for="password">Password</label>
