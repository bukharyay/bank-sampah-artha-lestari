<div class="main-panel">
                ...
                <form class="pt-3" method="POST" id="form_transaksi">
                  <div class="row justify-content-center w-auto mx-auto">
                    ...
                    <div class="col-12 col-sm-12 col-md-10 col-lg-6">
                      ...
                      <div class="form-group">
                        <label class="form-label">Harga Sampah <small>(/kg)</small></label>
                        <input type="text" class="form-control" id="berat_harga_sampah" placeholder="Pilih Sampah"
                          min="1" readonly>
                    ...
                    <div class="form-group">
                      <label class="form-label">Total Harga Sampah</label>
                      <input type="text" class="form-control" id="total_harga_sampah"
                        placeholder="Masukkan Berat Sampah" min="1" step="1" readonly>
                    </div>
  ...
  $(document).ready(function() {
    ...
    initSelect2('#id_sampah_sel', '<?= base_url ( 'Search-Sampah' ) ?>', 'Pilih Sampah');
		...
		// Variabel untuk menyimpan harga per kg
let hargaPerKg = 0;
...
function hitungTotalHarga() {
    const beratGram = parseFloat($('#berat').val()) || 0;
    const beratKg = beratGram / 1000; // Konversi gram ke kg
    const total = beratKg * hargaPerKg;
    
    $('#total_harga_sampah').val(formatRupiah(total.toFixed(2)));
}

// Ketika sampah dipilih
$('#id_sampah_sel').on('change', function() {
    $.ajax({
        type: "GET",
        url: '<?= base_url('Search-Harga-Sampah/') ?>' + $(this).val(),
        dataType: 'json',
    })
    .done(function(response) {
        if (response && response.harga_per_kg !== undefined) {
            hargaPerKg = parseFloat(response.harga_per_kg);
            $('#berat_harga_sampah').val(formatRupiah(hargaPerKg));
            $('#berat_harga_sampah').removeClass('is-invalid');
            hitungTotalHarga(); 
        } else {
            hargaPerKg = 0;
            $('#berat_harga_sampah').val('Rp. 0,00');
            $('#berat_harga_sampah').addClass('is-invalid');
            $('#total_harga_sampah').val('Rp. 0,00');
        }
    })
    .fail(function() {
        hargaPerKg = 0;
