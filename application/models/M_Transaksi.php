...
class M_Transaksi extends CI_Model
  ...
  public function history_terakhir ()
    {
    return $this->db->select ( '*' )
      ->from ( $this->tables[ 'transaksi' ] )
      ...
      // ->where ( "{$this->tables[ 'transaksi' ]}.status", 'dicatat' )
      // ->where_in ( "{$this->tables[ 'transaksi' ]}.status", [ 'diterima', 'dicatat' ] )
      ->order_by ( "{$this->tables[ 'transaksi' ]}.tanggal_transaksi", "DESC" )
      ->order_by ( "{$this->tables[ 'transaksi' ]}.kode_transaksi", "DESC" )
      ->limit ( 10 )
      ->get ()->result ();
    }
  // public function history_transaksi ( $laporan = null )
  //   {
  //   $this->db->select ( '*' )
  //     ->from ( $this->tables[ 'transaksi' ] )
  //     ->join ( $this->tables[ 'sampah' ], "{$this->tables[ 'transaksi' ]}.id_sampah = {$this->tables[ 'sampah' ]}.id_sampah" )
  ...
  //     {
  //     case 'selesai':
  //       $this->db->where_in ( "{$this->tables[ 'transaksi' ]}.status", "dibawa" );
  //       break;
  //     case 'proses':
  //       $this->db->where_in ( "{$this->tables[ 'transaksi' ]}.status", "dicatat" );
  //       break;
  //     case 'diterima':
  //       $this->db->where_in ( "{$this->tables[ 'transaksi' ]}.status", "diterima" );
  //       break;
  //     default:
  //       $this->db->where_in ( "{$this->tables[ 'transaksi' ]}.status", [ 'diterima', 'dicatat' ] );
  //       break;
  //     }
  //   // if ( $laporan != null )
  //   //   {
  //   //   $this->db->or_where_not_in ( "{$this->tables[ 'transaksi' ]}.status", "diterima" );
  //   //   }
  ...
  public function get_rt_rw_list ()
    ...
    $this->db->select ( 't.id_rt, t.rt, t.id_rw, w.rw, t.nama_ketua_pkk, t.created_at, t.updated_at' )
      ...
      ->order_by ( 't.rt', 'ASC' );
    return $this->db->get ()->result ();
    // return $this->db->join ( $this->tables[ 'rw' ] . ' w', 't.id_rw = w.id_rw' )->order_by ( 'rt', 'ASC' )->group_by ( 't.rt' )->get ( $this->tables[ 'rt' ] . ' t' )->result ();
    }
  public function generate_kode_transaksi ()
    ...
    if ( $query->num_rows () > 0 )
      ...
      $nomor     = (int) substr ( $last_kode, -5 ); // Ambil 5 digit terakhir
      $nomor++;
      }
    else
      {
