<?php foreach ($data_users as $us) : ?>
	<?php if ($us->role !== 'admin') : ?>
		<div class="modal fade" id="userEdit<?= $us->id_user ?>" tabindex="-1" aria-labelledby="userEdit<?= $us->id_user ?>Label" style="display: none;" aria-hidden="true">
			<div class="modal-dialog modal-lg" role="document">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title text-warning" id="userEdit<?= $us->id_user ?>Label">Edit Pengguna <i class="ti-pencil text-warning"></i></h5>
						<button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
							<span aria-hidden="true">×</span>
						</button>
					</div>
					<div class="modal-body">
						<form class="forms-sample" id="userEdit<?= $us->id_user ?>" action="<?= base_url('Edit-Users/') . $us->id_user ?>" method="POST" enctype="multipart/form-data">
							...
							<div class="row justify-content-center align-items-center w-auto mx-auto ">
								<div class="col-12 col-sm-12 col-md-10 col-lg-6">
									...
									<div class="text-center mb-3">
										<div class="d-flex justify-content-center ">
											<img id="selectedAvatar<?= $us->id_user ?>" src="<?= base_url('assets/profile/avatars/' . ($us->avatar ?? 'default.png')) ?>" class="rounded-circle" style="width: 100px; height: 100px; object-fit: cover;" alt="example placeholder" />
										</div>
										<?php if ($us->role == 'nasabah') : ?>
											<div class="d-flex justify-content-center mt-4">
												<div class="btn btn-sm btn-primary btn-rounded">
													...
													<input type="file" class="form-control d-none" id="avatar<?= $us->id_user ?>" name="avatar" onchange="displaySelectedImage(event, 'selectedAvatar<?= $us->id_user ?>')" />
												</div>
											</div>
								...
								<div class="col-12 col-sm-12 col-md-10 col-lg-6">
									<div class="form-group">
										...
										<input type="text" class="form-control" id="dibuat<?= $us->id_user ?>" value="<?= $us->role == 'nasabah' ? $CI->format_waktu($us->nasabah_created_at) : $CI->format_waktu($us->user_created_at) ?>" placeholder="" readonly>
									</div>
									<div class="form-group">
										<label for=" dibuat"><?= !empty($us->role) ? ucfirst($us->role) : '' ?> update terakhir </label>
										<input type="text" class="form-control" id="diupdate<?= $us->id_user ?>" value="<?= $us->role == 'nasabah' ? $CI->format_waktu($us->nasabah_updated_at) : $CI->format_waktu($us->user_updated_at) ?>" placeholder="" readonly>
									</div>
							...
							<div class="row justify-content-center w-auto mx-auto">
								...
								<div class="col-12 col-sm-12 col-md-10 col-lg-6">
									<div class="form-group <?= form_error('username') ? 'has-danger' : '' ?>  ">
										...
										<input type="text" class="form-control" id="username<?= $us->id_user ?>" name="username" value="<?= $us->username ?>" placeholder="Username" required>
										<?= form_error('username', '<label class="error mt-2 text-danger">', '</label>'); ?>
									</div>

									<div class="form-group <?= form_error('email') ? 'has-danger' : '' ?>">
										<label for="email">Email address</label>
										<input type="email" class="form-control" id="email<?= $us->id_user ?>" name="email" value="<?= $us->email ?>" placeholder="Email" required>
									...
									<?php if ($us->role == 'nasabah') : ?>
										<div class="form-group <?= form_error('password') ? 'has-danger' : '' ?>">
											...
											<input type="password" class="form-control" id="password<?= $us->id_user ?>" name="password" value="" placeholder="Password" autocomplete="on">
										...
										<div class="form-group <?= form_error('password_confirmation') ? 'has-danger' : '' ?>">
											...
											<input type="password" class="form-control " id="password_confirmation<?= $us->id_user ?>" name="password_confirmation" value="" placeholder="Confirm Password" autocomplete="on">
								...
								<div class="col-12 col-sm-12 col-md-10 col-lg-6">
									<?php if ($us->role == 'nasabah') : ?>
										...
										<div class="form-group <?= form_error('nama') ? 'has-danger' : '' ?>">
											...
											<input type="text" class="form-control" id="nama<?= $us->id_user ?>" name="nama" value="<?= $us->nama ?? $us->nama ?>" placeholder="Nama Lengkap" required>
										...
										<div class="form-group <?= form_error('no_telfon') ? 'has-danger' : '' ?>">
											...
											<input type="number" class="form-control" id="no_telfon<?= $us->id_user ?>" name="no_telfon" value="<?= $us->no_telfon ?? $us->no_telfon ?>" placeholder="No Telfon">
											<?= form_error('no_telfon', '<label class="error mt-2 text-danger">', '</label>'); ?>
										</div>

										<div class="form-group">
											<label for="alamat">Alamat <small>(optional)</small></label>
											<textarea class="form-control h-50" rows="5" name="alamat" id="alamat<?= $us->id_user ?>"><?= $us->alamat ?? $us->alamat ?></textarea>
										</div>
									<?php else : ?>
										<div class="form-group <?= form_error('password') ? 'has-danger' : '' ?>">
											<label for="password">Password</label>
